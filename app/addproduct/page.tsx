...
const AddProduct = () => {
  ...
  const onsubmit = async (products: CreateProductForm) => {
    try {
      if(id) await axios.patch(`http://10.10.50.59:3000/products/${id}`, products);
      else await axios.post('http://10.10.50.59:3000/products', products);
      reset();
      if(id) setResp('პროდუქტი წარმატებით დაედითდა');
      else setResp('პროდუქტი წარმატებით შეიქმნა');
    } catch (err) {
      console.log(err);
      setResp('ვერ ჩაეშვა');
    }
