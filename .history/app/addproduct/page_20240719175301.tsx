...
const AddProduct = () => {
  ...
  const onsubmit = async (products: CreateProductForm) => {
    try {
      if(id) {
        await axios.patch(`http://10.10.50.59:3000/products/${id}`, products);
      }else {
        await axios.post('http://10.10.50.59:3000/products', products);
      }
      reset();
      if(id) setResp("პროდუქტი წარმატებით დაედითდა")
        else setResp("პროდუქტი წარმატებით შეიქნა")
  ...
  return (
    <div className={styles.mainWrapper}>
      <form onSubmit={handleSubmit(onsubmit)} className={styles.formWrapper}>
        <h1>{id ? "Edit": "add New" }Add new fruit</h1>
